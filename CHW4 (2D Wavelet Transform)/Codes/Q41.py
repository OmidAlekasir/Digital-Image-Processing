...
def MSE(err):
    return np.sum(np.power(err, 2))/(np.size(img))
def ABS(img):
...
########################################################
coeffs=pywt.wavedec2(img_noisy, 'db4', mode=pywt.Modes.periodic, level=1)
...
# arr1[0:272][0:272]=denoise(arr1, 0,272,0,272)
...
plt.title(r'Noisy Image - $\sigma$=1')
plt.axis('off')
plt.subplot(133)
plt.imshow(denoised, cmap='gray')
plt.title('Denoised Image - DL=1')
plt.axis('off')
